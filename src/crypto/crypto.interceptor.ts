...
export class CryptoInterceptor implements NestInterceptor {
  ...
  intercept(ctx: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (!shouldDecrypt) {
      return next.handle(); // ← deja pasar tal cual
    }
    ...
    const prepare$ = from(this.crypto.decryptEnvelope(body)).pipe(
      tap(({ aesKey: k, nonce: n, plain }) => {
        aesKey = k;
        nonce = n;
        (req as any).body = plain; // el controller recibe JSON plano
      }),
      mapTo(void 0)
    );
    ...
    return prepare$.pipe(
      ...
      mergeMap((data) => {
        if (!aesKey) return of(data); // por seguridad (no debería ocurrir)
        return from(this.crypto.encryptResponse(aesKey, nonce, data)).pipe(
          tap(() => {
            res.header('X-Enc', '1');
            res.header('X-Nonce', nonce);
          }),
          // devolvemos el envelope cifrado
          map((env) => env),
        );
      }),
      catchError((err) => {
